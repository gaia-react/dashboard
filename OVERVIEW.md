...
Inspired by [ocodista/claude-usage](https://github.com/ocodista/claude-usage) and [phuryn/claude-usage](https://github.com/phuryn/claude-usage) — both parse `~/.claude` session logs into token/cost dashboards. GAIA Dashboard borrows their data-visualization ideas but narrows the focus to a single GAIA project: what did *this* project's specs and plans actually cost, not just raw Claude Code usage.
...
1. **GAIA's own cost tracking** (the project being visualized), under `.gaia/local/`:
   ...
   - `plans/**/*/cost.md` and `specs/**/*/cost.md` — human-readable per-plan/per-spec rollups (planning vs. execution phases, elapsed time, cost) rendered from the same underlying data. Useful as a secondary/legible source or fallback, but `cost.jsonl` should be preferred where both exist. Whether `cost.md` parsing is worth the maintenance cost (vs. relying solely on `cost.jsonl`) is an open call for whoever builds the parser.
...
- Zod 4 for parsing/validating the JSONL and cost.md data.
...
Project foundation (package.json, Vite/TS/Tailwind/lint scaffold, stub page) is in place. Data parsing, reconciliation, and the actual dashboard UI are not yet built — see the project plan (once written) for sequencing.
