...
export const buildManualApplyAgentAction = (eventOrId = 'EVENT_ID') => ({
...
export const clearManualApplyTransaction = (
  cwd = process.cwd(),
  transactionId = null
) => {
...
export const collectManualApplyFiles = (
  batch,
  extraFiles = [],
  cwd = process.cwd()
) => {
...
export const compactManualApplyBatch = (batch = {}, cwd = process.cwd()) => {
...
export const compactManualApplyCandidates = (candidates, cwd = process.cwd()) =>
...
export const compactManualLogText = (value, max = 200) => {
...
export const countManualApplyOps = (entriesOrBatch) => {
  const entries =
    Array.isArray(entriesOrBatch) ? entriesOrBatch
...
export const createManualApplyController = ({
  ...
  const tombstoneTimedOutApplyId = (eventId, details = {}) => {
    if (!eventId) return;
  ...
  const pushBatchInChunksAndWait = async (batch, pageUrl, context = {}) => {
    ...
    return {
      ...
      status:
        ...
        : appliedEntryIds.length > 0 ? 'partial'
        : 'error',
    };
  };

  const getDeferred = (eventId) => pendingApplyDeferreds.get(eventId) || null;

  const hasTimedOutId = (eventId) => timedOutApplyIds.has(eventId);

  const resolveDeferred = (eventId, body) => {
  ...
  const rejectDeferred = (eventId, reason) => {
    const deferred = pendingApplyDeferreds.get(eventId);
    if (!deferred) return false;
    pendingApplyDeferreds.delete(eventId);
    clearTimeout(deferred.timer);
  ...
  const referencedManualApplyEvidencePaths = (cwdValue = projectCwd()) => {
  ...
  const pruneStaleEvidence = (cwdValue = projectCwd()) => {
  ...
  const rollbackTimedOutReply = (message) => {
    const details = timedOutApplyIds.get(message.id);
    if (!details) return {rollbackFailures: [], rolledBackFiles: []};
    timedOutApplyIds.delete(message.id);
  ...
  const cancelPendingEvents = (pageUrl, reason = 'manual_edit_discarded') => {
    ...
    for (let index = pendingEvents.length - 1; index >= 0; index -= 1) {
      const event = pendingEvents[index]?.event;
  ...
  return {
    ...
    writeTransaction: (options = {}) =>
      writeManualApplyTransaction({cwd: projectCwd(), ...options}),
  };
};

export const manualApplyEvidenceDir = (cwd = process.cwd()) =>
  path.join(getLiveDir(cwd), 'manual-edit-evidence');

export const manualApplyTransactionPath = (cwd = process.cwd()) =>
  path.join(getLiveDir(cwd), 'manual-edit-apply-transaction.json');

export const manualEditApplyChunkSize = (env = process.env) => {
...
export const normalizeManualApplyEvidencePath = (
  evidencePath,
  cwd = process.cwd()
) => {
  if (!evidencePath || typeof evidencePath !== 'string') return null;
  const fullPath =
    path.isAbsolute(evidencePath) ? evidencePath : (
...
export const readManualApplyTransaction = (cwd = process.cwd()) => {
  const file = manualApplyTransactionPath(cwd);
...
export const removeManualApplyEvidence = (
  evidencePath,
  cwd = process.cwd()
) => {
...
export const rollbackApplySnapshot = (
  batch,
  rollbackSnapshot,
  extraFiles = [],
...
export const rollbackManualApplyTransaction = ({
  cwd = process.cwd(),
  pageUrl = null,
...
export const snapshotApplyEventFiles = (batch, cwd = process.cwd()) => {
...
export const splitManualApplyBatch = (batch, maxOps) => {
...
export const summarizeManualApplyEvent = (
  event = {},
  batch = event.batch,
  cwd = process.cwd()
) => {
...
export const summarizeManualApplyFailures = (failed, cwd = process.cwd()) => {
...
export const summarizeManualDiagnostics = (items, cwd = process.cwd()) => {
...
export const summarizeManualLogFile = (file, cwd = process.cwd()) => {
...
export const validateManualApplyResultMessage = (message, deferred) => {
  const data = message?.data;
  const eventId = message?.id || deferred?.event?.id || 'EVENT_ID';
...
export const writeManualApplyEvidence = (
  eventId,
  batch,
  cwd = process.cwd()
) => {
...
export const writeManualApplyTransaction = ({
  batch,
  cwd = process.cwd(),
  pageUrl = null,
}) => {
...
const addOpToManualApplyChunk = (chunk, entry, op) => {
...
const compactManualApplyContext = (value) => {
...
const compactManualApplyEntry = (entry = {}) => ({
...
const compactManualApplyOp = (op = {}) => ({
  classes: Array.isArray(op.classes) ? op.classes : [],
  container: compactManualApplyContext(op.container),
  contextHints:
    Array.isArray(op.contextHints) ? op.contextHints.slice(0, 8) : undefined,
...
const compactManualApplySourceMatch = (match, cwd) => {
...
const compactManualApplySourceMatches = (matches, limit, cwd) =>
  (Array.isArray(matches) ? matches : [])
    .slice(0, limit)
    .map((match) => compactManualApplySourceMatch(match, cwd))
    .filter(Boolean);

const compactNearbyManualEditTexts = (items) =>
...
const createManualApplyChunkBuilder = () => ({
  entries: [],
  entryById: new Map(),
  entryIds: new Set(),
  opCount: 0,
...
const filterManualApplyChunkCandidates = (batch, refsByEntry) =>
...
const firstFailureReason = (result) => {
  const first =
    Array.isArray(result?.failed) ? result.failed.find(Boolean) : null;

  return first?.reason || first?.message || null;
};

const invalidManualApplyResult = (reason, eventId, extra = {}) => ({
  body: {
...
const manualApplyReplyCommand = (eventOrId = 'EVENT_ID') => {
  const id =
...
const manualApplyResultShapeHint = (eventId = 'EVENT_ID') =>
  `Use live-poll.mjs --reply ${eventId} done --data '{"status":"done","appliedEntryIds":["ENTRY_ID"],"failed":[],"files":["src/page.html"],"notes":[]}'`;

const markChunkEntriesFailed = (failedByEntry, chunk, reason) => {
  for (const entryId of chunk.entryIds) {
    if (failedByEntry.has(entryId)) continue;
...
const normalizeApplyChunkResult = (result) => {
  const status =
  ...
  return {
    appliedEntryIds:
      Array.isArray(result?.appliedEntryIds) ?
...
const normalizeProjectFile = (file, cwd = process.cwd()) => {
...
const truncateManualApplyText = (value, max) => {
