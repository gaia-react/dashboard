...
export const createManualEditRoutes = ({
  ...
  return (req, res, url) => {
    ...
    if (p === '/manual-edit-stash' && req.method === 'POST') {
      ...
      req.on('end', () => {
        ...
        if (message.token !== getToken()) {
          sendJson(res, 401, {error: 'Unauthorized'});

          return;
        }
        const error = validateEvent({...message, type: 'manual_edits'});
        ...
        const {perPage, totalCount} = countPendingByPage(projectCwd());
        const pendingCount = perPage[message.pageUrl] || 0;
    ...
    if (p === '/manual-edit-commit' && req.method === 'POST') {
      ...
      const pendingCount =
        pageUrl ? before.perPage[pageUrl] || 0 : before.totalCount;
      ...
      (async () => {
        ...
        try {
          ...
          const envValue = currentEnvironment();
          ...
          if (useChatRoute) {
            ...
            const provider =
              ['claude', 'codex', 'mock'].includes(requestedMode) ?
                requestedMode
              : undefined;
            result = await commitManualEdits({
              batch: commitBatch,
        ...
        if (result?.needsManualDecision) {
          ...
          recordManualEditActivity('manual_edit_commit_done', {
            ...
            rolledBackFiles:
              Array.isArray(result.rolledBackFiles) ?
            ...
            totalCount,
            unreportedFiles:
              Array.isArray(result.unreportedFiles) ?
...
const summarizePendingManualEditBatch = (cwd, pageUrl = null) => {
  try {
