...
const parseRuleList = (remainder) => {
  let text = String(remainder || '')
    .replace(TRAILING_CLOSER_RE, '')
    .trim();
  // Cut off a human reason at the first `--` (eslint) or `:` (biome) separator.
  const reasonSeparator = text.match(/\s*(?:--+|:)\s*/);
  if (reasonSeparator) text = text.slice(0, reasonSeparator.index);
  const tokens = text
    .split(/[\s,]+/)
    .map(normalizeRule)
    .filter(Boolean);
 ...
 */
const parseInlineIgnores = (content) => {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    while ((m = DIRECTIVE_RE.exec(line)) !== null) {
      ...
      if (variant === 'disable') {
        ...
        addRules(getSet(result.line, index + 1), rules);
      } else {
        // disable-next-line on line i+1 targets line i+2.
        addRules(getSet(result.nextLine, index + 2), rules);
      }
    }
  }
...
const setMatches = (set, rule) =>
  Boolean(set) && (set.has('*') || set.has(rule));

const isInlineIgnored = (finding, directives) => {
 ...
 */
const applyInlineIgnores = (findings, content) => {
