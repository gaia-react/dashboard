...
const RULE_ENGINE_SUPPORT = {
  browser: new Set(['element', 'layout', 'page']),
  regex: new Set(['page-analyzer', 'source']),
  'static-html': new Set(['element', 'page']),
  visual: new Set(['visual-contrast']),
};

const getAntipattern = (id) => ANTIPATTERNS.find((rule) => rule.id === id);

const getRulesForCategory = (category) =>
  ANTIPATTERNS.filter((rule) => rule.category === category);

const getRuleEngineSupport = (engine) =>
  RULE_ENGINE_SUPPORT[engine] || new Set();
...
const filterByProviders = (findings, providers = []) => {
