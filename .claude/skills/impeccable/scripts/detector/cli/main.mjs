...
const formatFindingSummary = (count) =>
  `${count} anti-pattern${count === 1 ? '' : 's'} found.`;

const formatFindings = (findings, jsonMode) => {
...
const handleStdin = async (options = {}) => {
  const chunks = [];
...
const detectCli = async () => {
  let args = process.argv.slice(2).map((argument) => {
    if (argument === '-json') return '--json';
    if (argument === '-fast') return '--fast';
  ...
  const designSystem =
    designSystemEnabled ? loadDesignSystemForCwd(process.cwd()) : null;
