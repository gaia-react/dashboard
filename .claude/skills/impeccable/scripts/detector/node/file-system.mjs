...
const resolveImport = (specifier, fromDir, fileSet) => {
  ...
  for (const extension of SCANNABLE_EXTENSIONS) {
    ...
    if (fileSet.has(withExtension)) return withExtension;
  }
...
const detectFrameworkConfig = (dir) => {
  let entries;
 ...
 */
const isPortListening = async (port, fingerprint = null) => {
  if (!fingerprint) {
  ...
  try {
    ...
    if (fingerprint.header) {
      const value = res.headers.get(fingerprint.header);

      if (value && (!fingerprint.value || fingerprint.value.test(value))) {
