 ...
 */
export const detectCsp = (cwd = process.cwd()) => {
  const hits = {
    appendArrays: [],
    appendString: [],
    metaTag: [],
  ...
  walk(cwd, cwd, 0, (absPath, relPath, body) => {
    ...
    if (LAYOUT_EXTS.has(extension) && META_TAG_HINT.test(body)) {
...
const walk = (root, dir, depth, visit) => {
  ...
  for (const entry of entries) {
    ...
    if (!SCAN_EXTS.has(extension) && !LAYOUT_EXTS.has(extension)) continue;
    let body;
