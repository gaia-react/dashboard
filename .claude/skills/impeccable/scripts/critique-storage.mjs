 ...
 */
export const nowFilenameStamp = (date = new Date()) => {
 ...
 */
export const readLatestSnapshot = (slug, {cwd = process.cwd()} = {}) => {
 ...
 */
export const readTrend = (slug, {cwd = process.cwd(), limit = 5} = {}) => {
  const all = listSnapshotsForSlug(slug, cwd);
 ...
 * phrase.
 */
export const slugFromTarget = (resolved, {cwd = process.cwd()} = {}) => {
 ...
 */
const listSnapshotsForSlug = (slug, cwd) => {
  const dir = getCritiqueDir(cwd);
...
const main = (argv) => {
  ...
  switch (cmd) {
    ...
    case 'write': {
      ...
      let meta = {};
      const metaArgument = process.env.IMPECCABLE_CRITIQUE_META;

      if (metaArgument) {
        try {
          meta = JSON.parse(metaArgument);
        } catch {
          /* ignore */
        }
      }
...
const serializeFrontmatter = (obj) => {
  const lines = ['---'];
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    const needsQuotes = typeof value === 'string' && /[:#]/.test(string_);
    lines.push(`${key}: ${needsQuotes ? JSON.stringify(string_) : string_}`);
  }
  lines.push('---');
