...
const fetchServerStatus = async (info) => {
  if (!info) return null;
...
export const statusCli = async () => {
  ...
  const payload = {
    ...
    liveServer:
      ...
      : null,
    recoveryHint:
      manualApply ? manualApplyResumeHint(manualApply)
      : server ?
...
const findPendingManualApply = (server, activeSessions) => {
