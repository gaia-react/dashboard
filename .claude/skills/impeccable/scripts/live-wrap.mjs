...
The agent should insert variant HTML at insertLine.`);
  ...
  const count = Number.parseInt(argumentValue(args, '--count') || '3');
  const elementId = argumentValue(args, '--element-id');
  const classes = argumentValue(args, '--classes');
  const tag = argumentValue(args, '--tag');
  const query = argumentValue(args, '--query');
  const filePath = argumentValue(args, '--file');
  const text = argumentValue(args, '--text');
  ...
  const genOptions = {cwd: process.cwd()};
  ...
  if (targetFile) {
    if (isGeneratedFile(targetFile, genOptions)) {
      console.error(
        JSON.stringify({
    ...
    for (const q of queries) {
      targetFile = findFileWithQuery(q, process.cwd(), genOptions);
  ...
  const reindentOriginal = (extra) =>
    originalLines
      ...
      .join('\n');
  const originalIndented = reindentOriginal(' '.repeat(4));
  ...
  const svelteComponentAuthoring =
    useSvelteComponent ? buildSvelteComponentCssAuthoring(count) : null;
...
const pendingEntriesThatMayAffectWrap = (
  entries,
  targetFile,
  originalLines,
  selectionStartLine,
...
const manualEditHintFallsInsideSelection = (
  op,
  targetAbs,
  originalLines,
  selectionStartLine,
  ...
  const hintAbs =
    path.isAbsolute(hintFile) ? hintFile : path.resolve(cwd, hintFile);
...
const manualEditLocatorMatchesSelection = (op, originalLines) => {
  if (
    !op ||
    typeof op.originalText !== 'string' ||
...
const applyBufferedManualEditToLines = (
  originalLines,
  selectionStartLine,
  op
) => {
...
const replaceOnce = (value, needle, replacement) => {
 ...
 */
const buildSearchQueries = (elementId, classes, tag, query) => {
  const queries = [];
  ...
  if (classes) {
    ...
    if (classList.length > 1) {
      ...
      queries.push(`class="${joined}"`, `className="${joined}"`);
  ...
  if (tag && classes) {
    ...
    queries.push(
      `<${tag} class="${firstClass}`,
      `<${tag} className="${firstClass}`
    );
  }
...
const splitClassList = (classes) =>
  String(classes)
    .split(/[,\s]+/)
    .map((c) => c.trim())
    .filter(Boolean);

const attributeEscapeDouble = (string_) =>
  String(string_)
    .replaceAll('&', '&amp;')
    .replaceAll('"', '&quot;')
    .replaceAll('<', '&lt;')
...
const detectStyleMode = (filePath) => {
  const extension = path.extname(filePath).toLowerCase();
...
const buildCssSelectorPrefixExamples = (styleMode, count) => {
  if (styleMode !== 'astro-global-prefixed') return [];

  return Array.from(
    {length: count},
    (_, index) => `[data-impeccable-variant="${index + 1}"]`
  );
};

const buildCssAuthoring = (styleMode, count) => {
 ...
 */
const findFileWithQuery = (query, cwd, genOptions = {}) => {
  ...
  for (const dir of searchDirectories) {
    ...
    const result = searchDir(absDir, query, seen, 0, genOptions);
...
const searchDir = (dir, query, seen, depth, genOptions) => {
  ...
  for (const entry of entries) {
    ...
    if (!genOptions.includeGenerated && isGeneratedFile(filePath, genOptions))
      continue;
...
const findElement = (lines, query, tag = null) => {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (lines[index].includes('data-impeccable-variant')) continue;

    const openerLine = findOpenerLine(lines, index, tag);
 ...
 */
const findAllElements = (lines, query, tag = null) => {
  const out = [];
  const seen = new Set();
 ...
 * back to first-match.
 */
const filterByText = (candidates, lines, text) => {
  const trimmed = text
    .replaceAll(/\s+/g, ' ')
    .trim()
    .toLowerCase()
 ...
 */
const findOpenerLine = (lines, matchLine, tag) => {
  ...
  for (
    let index = matchLine - 1;
    index >= Math.max(0, matchLine - MAX_BACKWALK);
    index--
  ) {
    const opener = lines[index].match(OPENER_RE);
    if (!opener) continue;
    if (!tag || opener[1] === tag) return index;
