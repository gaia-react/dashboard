 ...
 */
const findHarnessDirectories = (projectRoot) => {
  const directories = [];
 ...
 */
const generatePinnedSkill = (command, metadata) => {
  const desc =
 ...
 */
const pin = (command, projectRoot) => {
  const metadata = loadCommandMetadata();
  const harnessDirectories = findHarnessDirectories(projectRoot);

  if (harnessDirectories.length === 0) {
  ...
  for (const skillsDir of harnessDirectories) {
 ...
 */
const unpin = (command, projectRoot) => {
  const harnessDirectories = findHarnessDirectories(projectRoot);
  let removed = 0;
