...
const gitSignals = (cwd) => {
  ...
  const fromDiff =
    diffBase ? run(['diff', '--name-only', `${diffBase}...HEAD`]) : null;
...
const latestCritique = (cwd) => {
  try {
    ...
    const front = text.split('---', 2)[1] || '';

    const get = (k) => {
      const m = front.match(new RegExp(String.raw`^${k}:\s*(.+)$`, 'm'));
...
const developmentServerSignals = async () => {
  const open = [];
  await Promise.all(
...
const probePort = (port, timeout = 250) =>
  new Promise((resolve) => {
...
export const gatherSignals = async (cwd = process.cwd()) => {
  const ctx = loadContext(cwd);
  const git = gitSignals(cwd);

  return {
    critique: {latest: latestCritique(cwd)},
    devServer: await developmentServerSignals(),
    git,
...
const scanTargets = (cwd, git) => {
  ...
  const directories = SOURCE_DIRS.filter((d) =>
    fs.existsSync(path.join(cwd, d))
  );
  if (directories.length > 0) return {targets: directories, via: 'source-dir'};
