...
export const bumpEditCount = (cache, sessionId, filePath) => {
...
export const dedupeAgainstCache = (findings, cache, sessionId, filePath) => {
...
export const ensureHookGitExcludes = (cwd = process.cwd()) => {
  try {
    ...
    const existing =
      fs.existsSync(target.path) ? fs.readFileSync(target.path, 'utf-8') : '';
    const block = [markerOpen, ...patterns, markerClose].join('\n');
    const markerRe = new RegExp(String.raw`${escapeRegExp(markerOpen)}[\s\S]*?${escapeRegExp(markerClose)}`);
...
export const filterFindings = (findings, _content, _extension, config) => {
  if (!Array.isArray(findings) || findings.length === 0) return [];
...
export const getCachePath = (cwd) =>
  path.join(cwd, '.impeccable', 'hook.cache.json');

export const getConfigPath = (cwd) =>
  path.join(cwd, '.impeccable', 'config.json');

export const getLocalConfigPath = (cwd) =>
  path.join(cwd, '.impeccable', 'config.local.json');

export const getPendingPath = (cwd) =>
  path.join(cwd, '.impeccable', 'hook.pending.json');

export const matchConfiguredExtension = (filePath, extensions) => {
...
export const persistCache = (cwd, cache) => {
...
export const rememberFindings = (cache, sessionId, filePath, findings) => {
  const fileEntry = ensureFile(cache, sessionId, filePath);
  const known = new Set(fileEntry.findings || []);
  for (const f of findings) known.add(findingCacheKey(f));
  fileEntry.findings = [...known];
  ensureSession(cache, sessionId).updatedAt = Date.now();
};

export const renderTemplate = (findings, filePath, config, options = {}) => {
  if (!Array.isArray(findings) || findings.length === 0) return '';
...
export const resolveCacheCwd = (primaryFile, sessionCwd) => {
...
export const resolveProjectCwd = (event, fallback = process.cwd()) =>
  event?.cwd ||
  (Array.isArray(event?.workspace_roots) && event.workspace_roots[0]) ||
  envProjectDir(fallback) ||
  fallback;

export const suppressionNotice = (filePath) =>
  `${ENVELOPE_PREFIX} Suppressing further design hints on ${filePath}. More than ${EDIT_COUNT_THRESHOLD} edits in this session reached. Run /impeccable audit to revisit.`;

export const truthy = (value) =>
  typeof value === 'string' && TRUTHY.test(value);

const applyConfigSource = (config, raw) => {
...
const applyDetectorConfigSource = (config, raw) => {
  if (!raw || typeof raw !== 'object') return config;
...
const clampGroupedToBudget = (header, lines, footer, maxChars) => {
  const assemble = (linesArray, omitted) => [
    header,
    ...linesArray,
...
const clampToBudget = (header, lines, more, footer, maxChars) => {
  const assemble = (linesArray, moreText) => {
    const blocks = [header, ...linesArray];
...
const ensureFile = (cache, sessionId, filePath) => {
...
const ensureSession = (cache, sessionId) => {
...
const formatFindingIgnoreCommand = (finding) => {
  if (!finding || typeof finding !== 'object') return '';
  const rule = normalizeIgnoreRule(finding.antipattern);
  if (!rule) return '';
  ...
  const valueArgument = quoteCommandArg(value);
  const reason = quoteCommandArg(`User confirmed ${value} is intentional`);
  return `/impeccable hooks ignore-value ${rule} ${valueArgument} --shared --reason ${reason}`;
};

const formatFindingLine = (f) => {
...
const globToRegex = (glob) => {
  ...
  while (index < glob.length) {
    ...
    if (c === '*') {
      ...
      if (end === -1) { re += '\\{'; index += 1; continue; }
      const parts = glob.slice(index + 1, end).split(',').map((p) => p.replace(/[.+^$()|[\]\\]/g, '\\$&'));
      re += `(?:${parts.join('|')})`;
      index = end + 1;
    } else if (/[.+^$()|[\]\\]/.test(c)) {
      re += `\\${c}`;
...
const ignoreValueFilesKey = (files) =>
  Array.isArray(files) && files.length > 0 ? files.join('\x1F') : '';
...
const looksLikeProjectRoot = (dir) =>
  ['.git', 'package.json', '.impeccable'].some((marker) => {
    try {
...
const mergeExtensions = (existing, incoming) => {
  const map = new Map();
...
const parseIgnoreColor = (value) => {
  ...
  if (rgb) {
    ...
    if ([r, g, b, a].includes(null)) return null;
  ...
  if (hsl) {
    ...
    const a = parts[3] === undefined ? 1 : parseAlphaChannel(parts[3]);
    if ([h, s, l, a].includes(null)) return null;
...
const renderGroupedTemplate = (groups, config, options = {}) => {
  ...
  if (realGroups.length === 1) {
    ...
    return renderTemplate(group.findings, group.filePath, config, options);
  }
...
const resolveGitDir = (dotGit, worktreeDir) => {
...
const resolveHookGitExcludeTarget = (cwd) => {
...
export const parseApplyPatchPaths = (command, projectCwd) => {
...
export const parseGitHubToolArgs = (toolArgs) => {
...
export const resolveHarness = (env = {}, event = null) => {
...
export const resolveTargetFiles = (event, projectCwd) => {
...
export const normalizeHookEvent = (event, projectCwd, harness = 'claude') => {
...
const looksLikeApplyPatch = (rawArgs) => {
...
const normalizeGitHubEvent = (event, projectCwd) => {
...
const UI_CODE_EXTS = new Set(['.astro', '.jsx', '.svelte', '.tsx', '.vue']);
const STYLE_EXTS = new Set(['.css', '.less', '.sass', '.scss']);
...
export const coLocatedStylesheets = (filePath) => {
...
export const expandScanTargets = (primaryTargets, projectCwd) => {
  ...
  for (const p of normalizedPrimaries) {
    ...
    if (STYLE_EXTS.has(extension) || !UI_CODE_EXTS.has(extension)) continue;
...
export const normalizeScanTargets = (primaryTargets, projectCwd) => {
...
export const parseStaticStyleImports = (content, fromFile, projectCwd) => {
...
export const writeAuditLog = (env, entry, cwd = process.cwd()) => {
...
const hasPathTraversal = (filePath) =>
  typeof filePath === 'string' && filePath.includes('..');

const isInsideProject = (filePath, projectCwd) => {
...
export const loadDetector = async (candidates = DETECTOR_CANDIDATES) => {
  if (detectorCache) return detectorCache;
  const found = candidates.find((c) => fs.existsSync(c));
  if (!found) return null;
  const module_ = await import(pathToFileURL(found));
  detectorCache = {
    detectText: module_.detectText,
    detectHtml: module_.detectHtml,
    loadDesignSystemForCwd: module_.loadDesignSystemForCwd,
  };
...
export const setDetectorForTesting = (impl) => {
  detectorCache = impl;
};
...
export const appendDesignSystemNote = (text, scanOptions) => {
...
export const designSystemOptions = (config, detector, projectCwd) => {
...
export const renderCleanAck = (filePath, options = {}) => {
  const cwd = options.cwd || process.cwd();
  const display = relativize(filePath, cwd);
...
export const renderPendingAck = (filePath, knownFindings, options = {}) => {
  const cwd = options.cwd || process.cwd();
  const display = relativize(filePath, cwd);
...
export const runHook = async ({
  ...
  try {
    ...
    for (const filePath of targetFiles) {
      ...
      audit.ext = configuredExt ? configuredExt.ext : extension;
      if (!ALLOWED_EXTS.has(extension) && !configuredExt) {
        lastSkip = 'extension';
        continue;
      }
      ...
      if (filtered.length > 0 && !pendingWinner) {
        const known = [...(ensureFile(cache, sessionId, filePath).findings || [])];
    ...
    return {
      exitCode: 0,
      stdout: '',
      audit: { ...audit, error: String(error && error.message ? error.message : error) },
    };
  }
};

export const shouldEmitAckForFile = (filePath, config = null) => {
...
const directiveFooter = (display, options = {}) => {
  const ignoreFileCommand = `/impeccable hooks ignore-file ${quoteCommandArg(display)}`;
  const fileIgnoreGuidance = options.grouped
