...
const parseArgs = (argv) => {
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    if (a === '--id' && argv[index + 1]) {
      args.id = argv[++index];
    } else if (a === '--from' && argv[index + 1]) {
...
const weightedPick = (seeds, unit) => {
  const {total, weights} = buildWeights(seeds);
  let target = unit * total;

  for (const [index, seed_] of seeds.entries()) {
    target -= weights[index];
    if (target < 0) return seed_;
  }

  return seeds.at(-1);
};

const pickSeed = (seeds, {from, id}) => {
  if (id) {
