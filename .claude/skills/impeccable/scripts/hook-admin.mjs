...
const readRawConfigFile = (filePath) => {
...
const readRawHookConfig = (cwd, options = {}) => {
  const unified = readRawConfigFile(options.local ? getLocalConfigPath(cwd) : getConfigPath(cwd)).raw;

  return hookSection(unified);
};

const readRawDetectorConfig = (cwd, options = {}) => {
  const unified = readRawConfigFile(options.local ? getLocalConfigPath(cwd) : getConfigPath(cwd)).raw;
...
const writeHookConfig = (cwd, hookConfig, options = {}) => {
  const filePath = options.local ? getLocalConfigPath(cwd) : getConfigPath(cwd);
  if (options.local) ensureHookGitExcludes(cwd);
...
const writeDetectorConfig = (cwd, detectorConfig, options = {}) => {
  const filePath = options.local ? getLocalConfigPath(cwd) : getConfigPath(cwd);
  if (options.local) ensureHookGitExcludes(cwd);
  const existingRaw = readRawConfigFile(filePath).raw;
  const existing =
...
const mergeHookConfig = (existing) => {
...
const mergeDetectorConfig = (existing, seed = null) => {
  const base = existing && typeof existing === 'object' ? existing : {};
  const out =
    seed ?
      {
  ...
  if (Array.isArray(base.ignoreRules)) {
    out.ignoreRules = [...new Set([...out.ignoreRules, ...base.ignoreRules.map(String)])];
  }

  if (Array.isArray(base.ignoreFiles)) {
    out.ignoreFiles = [...new Set([...out.ignoreFiles, ...base.ignoreFiles.map(String)])];
  }
...
const mergeIgnoreValueEntries = (existing, incoming) => {
  const map = new Map();
...
const ignoreValueEntryKey = (entry) => {
  const files =
...
const statusReport = (cwd) => {
  ...
  const lines = [
    ...
    `  ignoreRules:  ${cfg.ignoreRules.length > 0 ? cfg.ignoreRules.join(', ') : '(none)'}`,
    `  ignoreFiles:  ${cfg.ignoreFiles.length > 0 ? cfg.ignoreFiles.join(', ') : '(none)'}`,
    `  ignoreValues: ${ignoreValues.length > 0 ? ignoreValues.join(', ') : '(none)'}`,
...
const repairHookManifests = (cwd) => {
  ...
  for (const target of HOOK_MANIFEST_TARGETS) {
    ...
    const destination = path.join(cwd, target.destRel);
    ...
    if (sharedDest && fileHasImpeccableHookMarker(sharedDest)) {
      pruneImpeccableHookFromManifest(destination);
    ...
    if (fs.existsSync(destination)) {
      try {
        next = mergeHookManifests(JSON.parse(fs.readFileSync(destination, 'utf-8')), fresh);
      } catch {
    ...
    const current = fs.existsSync(destination) ? safeReadText(destination) : null;
...
const mergeHookManifests = (existing, fresh) => {
...
const fileHasImpeccableHookMarker = (filePath) => {
...
const valueHasImpeccableHookMarker = (value) => {
...
const stripImpeccableHookEntry = (entry) => {
...
const stripImpeccableHookEntries = (entries) => {
  if (!Array.isArray(entries)) return [];

  return entries.map(stripImpeccableHookEntry).filter(Boolean);
};

const pruneImpeccableHookFromManifest = (manifestPath) => {
...
const normalizeRuleId = (rule) =>
  String(rule || '')
    .trim()
    .toLowerCase();

const parseIgnoreRuleArgs = (args) => {
  const positionals = [];
  let allValues = false;
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    if (arg === '--all-values') {
      ...
      while (index + 1 < args.length && !String(args[index + 1]).startsWith('--')) index++;
...
const parseIgnoreValueArgs = (args) => {
  const positionals = [];
  let shared = false;
  let local = false;
  let reason = '';
...
const addIgnoreValue = (cwd, args) => {
  ...
  const scope =
    local ? 'local detector.ignoreValues' : 'shared detector.ignoreValues';
...
const main = () => {
  const [, , actionArgument, ...rest] = process.argv;
  const action = (actionArgument || 'status').toLowerCase();
