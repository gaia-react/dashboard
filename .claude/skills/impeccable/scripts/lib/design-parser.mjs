...
const parseFrontmatter = (md) => {
  ...
  if (lines[0]?.trim() !== '---') return {body: md, frontmatter: null};
...
const normalizeApostrophes = (s) => s.replaceAll(/[\u2018\u2019]/g, "'");

const matchCanonicalSection = (name) => {
  ...
  for (const c of CANONICAL_SECTIONS) {
    ...
    const pattern = new RegExp(
      String.raw`\b${key.replaceAll(/[.*+?^${}()|[\]\\]/g, String.raw`\$&`)}\b`
    );
...
const stripBold = (s) => s.replaceAll(/\*\*(.+?)\*\*/g, '$1');

const extractNamedRules = (lines) => {
  const rules = [];
  const seen = new Set();
  ...
  for (let index = 0; index < inlineMatches.length; index++) {
    const mm = inlineMatches[index];
    const bodyEnd =
      index + 1 < inlineMatches.length ?
        inlineMatches[index + 1].start
      : joined.length;
    const body = joined
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (!h3) continue;
    const headerName = stripBold(h3[1]).replaceAll(/["“”]/g, '').trim();
    ...
    for (let index_ = index + 1; index_ < lines.length; index_++) {
      if (/^##\s|^###\s/.test(lines[index_])) break;
      bodyLines.push(lines[index_]);
    }
    ...
    if (flat.length > 0) {
      for (const p of flat) {
        if (p.name && ROLE_KEYWORDS.test(p.name)) {
          ...
          else groups.push({colors: [p], role: 'Palette'});
        }
      }
    }
  }
...
const extractParenGroup = (s) => {
  ...
  for (let index = 0; index < s.length; index++) {
    if (s[index] === '(') depth++;
      ...
      depth--;
      if (depth === 0) return s.slice(1, index);
    }
  }
...
const buildColor = (name, rawValue, description) => {
...
const collectColorValues = (s) => {
  const out = [];
  s.replaceAll(HEX_RE, (v) => {
    out.push(v);

    return v;
  });
  s.replaceAll(OKLCH_RE, (v) => {
    out.push(v);
...
const parseStitchInlineGroups = (lines) => {
...
const extractTypography = (section) => {
  if (!section) return null;
  const text = section.lines.join('\n');
  ...
  let character =
    characterMatch ? characterMatch[1].replaceAll('\n', ' ').trim() : null;
...
const extractElevation = (section) => {
  if (!section) return null;
  const subs = splitSubsections(section.lines);
...
const extractInlineShadows = (text) => {
...
const parseShadowBullet = (bullet) => {
...
const extractComponents = (section) => {
  if (!section) return null;
  const subs = splitSubsections(section.lines);
  const components = [];
...
export const parseDesignMd = (md) => {
  const {body, frontmatter} = parseFrontmatter(md);
  const {sections, title} = splitSections(body);
