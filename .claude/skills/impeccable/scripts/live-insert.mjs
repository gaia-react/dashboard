...
export const buildInsertWrapperLines = ({
  commentSyntax,
  count,
  id,
  indent,
...
export const computeInsertLine = (startLine, endLine, position) =>
  position === 'before' ? startLine : endLine + 1;

export const insertCli = async () => {
...
Output (JSON):
  ...
  const count = Number.parseInt(argumentValue(args, '--count') || '3', 10);
  const position = argumentValue(args, '--position');
  ...
  } else if (isGeneratedFile(targetFile, genOptions)) {
    console.error(
      JSON.stringify({
  ...
  const {endLine, startLine} = resolved.match;
...
export const isInsertPosition = (value) => INSERT_POSITIONS.has(value);
...
const resolveElementMatch = ({lines, queries, tag, text}) => {
  if (text) {
    const candidates = [];
