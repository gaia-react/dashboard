...
export const manualApplyResumeHint = (event = {}) => {
...
const summarizeManualApplyEvent = (event = {}) => {
  const entries =
    Array.isArray(event.batch?.entries) ? event.batch.entries : [];
...
const collectManualApplyFiles = (batch) => {
  const files = [];
...
const parseArgs = (argv) => {
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    if (argument === '--id') out.id = argv[++index];
    else if (argument.startsWith('--id='))
      out.id = argument.slice('--id='.length);
