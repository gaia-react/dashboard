...
export const buildCopyEditBatchPrompt = (batch, {cwd = process.cwd()} = {}) => {
  const repairLines =
    batch?.repair ?
      [
        '',
...
export const chooseCopyEditAgent = ({
 ...
 */
export const describeNoProviderError = ({
  chatAvailable = () => false,
  env = process.env,
 ...
 */
export const extractRunnerErrorMessage = (output, command) => {
...
export const parseCopyEditAgentResult = (text) => {
...
export const parseCopyEditBatchResult = (text) => {
...
export const runCopyEditBatchAgent = async (batch, options = {}) => {
  const cwd = options.cwd || process.cwd();
  const env = options.env || process.env;
  const provider =
    options.provider ||
    chooseCopyEditAgent({chatAvailable: options.chatAvailable, env});
  ...
  if (provider === 'chat') {
    if (typeof options.applyBatchToSource !== 'function') {
      throw new TypeError('chat provider requires applyBatchToSource callback');
    }
    const raw = await options.applyBatchToSource(batch, {
  ...
  const output =
    fs.existsSync(resultPath) ? fs.readFileSync(resultPath, 'utf-8') : '';
...
export const runCopyEditPostApplyChecks = ({
  cwd = process.cwd(),
  files = [],
} = {}) => {
  const failures = [];
...
const checkFrameworkSourceSyntax = (relativeFile, content) => {
...
const compactBatchForPrompt = (batch) => ({
...
const compactBatchOp = (op) => ({
  classes: op.classes,
  container: compactContextForBatch(op.container),
  contextHints:
    Array.isArray(op.contextHints) ? op.contextHints.slice(0, 12) : [],
  ...
  nearbyEditableTexts:
    Array.isArray(op.nearbyEditableTexts) ?
...
const compactContextForBatch = (value) => {
...
const findLeftoverImpeccableMarker = (content) => {
...
const isInsideQuotedLiteral = (line, index) => {
  let quote = null;
  let escaped = false;

  for (let index_ = 0; index_ < index; index_++) {
    const ch = line[index_];
...
const isPathInsideOrEqual = (cwd, file) => {
...
const mockBatchResult = (batch, env, cwd = process.cwd()) => {
...
const readManualEditValidationScript = (cwd) => {
  const packagePath = path.join(cwd, 'package.json');
  if (!fs.existsSync(packagePath)) return null;

  try {
    const package_ = JSON.parse(fs.readFileSync(packagePath, 'utf-8'));
    const script = package_?.scripts?.['impeccable:manual-edit-validate'];
...
const runAgentProcess = (
  command,
  args,
  stdin,
  {cwd, env, logPath, mirrorOutputPath, timeoutMs}
) =>
...
const runClaude = (
  prompt,
  {cwd, env, logPath, resultPath, timeoutMs = DEFAULT_TIMEOUT_MS}
) => {
  const args = [
    '--print',
    '--permission-mode',
...
const runManualEditValidationScript = (cwd) => {
...
const stripLiveRuntimeHtml = (html) => {
...
const computeCommandAuthed = (command) => {
