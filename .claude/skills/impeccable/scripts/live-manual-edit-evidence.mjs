...
export const buildManualEditEvidence = ({
  cwd = process.cwd(),
  pageUrl = null,
} = {}) => {
  ...
  for (const entry of entries) {
    ...
    for (const op of entry.ops || []) {
      out.push({
        ...
        leaf: op.leaf || null,
        nearbyEditableTexts:
          Array.isArray(op.nearbyEditableTexts) ? op.nearbyEditableTexts : [],
        newText: op.newText,
...
const buildContextHintsByRef = (entry) => {
  const map = new Map();
...
const buildCandidatesForOp = (op, cwd, searchFiles) => {
...
const literalMatchLimit = (text) =>
  isWeakSourceNeedle(text) ?
    WEAK_LITERAL_MATCH_LIMIT
  : STRONG_LITERAL_MATCH_LIMIT;

const isWeakSourceNeedle = (text) => {
  const normalized = normalizeText(text);

  return normalized.length < 4 || /^[\d.,+\-%\s]+$/.test(normalized);
};

const analyzeSourceHint = (op, cwd) => {
...
const normalizeSourceHint = (hint) => {
  if (!hint || typeof hint !== 'object') return {};
  let line = Number.isFinite(Number(hint.line)) ? Number(hint.line) : null;
  let column =
    Number.isFinite(Number(hint.column)) ? Number(hint.column) : null;
...
const collectSearchFiles = (cwd) => {
  const out = [];
  const seenDirectories = new Set();
  const seenFiles = new Set();

  for (const dir of SEARCH_DIRS) {
    scanDir(path.join(cwd, dir), cwd, seenDirectories, seenFiles, out, 0);
  }
...
const scanDir = (dir, cwd, seenDirectories, seenFiles, out, depth) => {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      scanDir(fullPath, cwd, seenDirectories, seenFiles, out, depth + 1);
      continue;
    }
    if (
      !entry.isFile() ||
...
const scanRootFiles = (cwd, seenFiles, out) => {
  let entries;
...
const maybeAddSearchFile = (file, cwd, seenFiles, out) => {
  let realFile;
...
const findLiteralMatches = (searchFiles, needle, {max}) =>
  findMatches(searchFiles, needle, {kind: 'text', max});

const findObjectKeyMatches = (searchFiles, text, {max}) => {
...
const findLocatorMatches = (searchFiles, op, {max}) => {
  const needles = [];
...
const findContextMatches = (searchFiles, hints, {max, maxPerHint}) => {
  const out = [];
  const seen = new Set();
...
const findMatches = (searchFiles, needle, {kind, max}) => {
...
const matchForIndex = (file, index, kind, needle) => {
